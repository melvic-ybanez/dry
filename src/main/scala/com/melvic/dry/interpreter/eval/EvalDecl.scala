...
private[eval] trait EvalDecl extends EvalStmt {
  ...
  def defDecl(implicit context: Context[Def]): Out =
    env.defineWith(node.name.lexeme, Callable.Function(node, _, locals, sourcePaths, isInit = false)).unit.ok
    ...
    node match {
      case ClassDecl(name, methods) =>
        ...
        val klass = DClass(
          ...
          methods
            .map(method =>
              method.name.lexeme -> Callable
                .Function(method, env, locals, sourcePaths, method.name.lexeme == Lexemes.Init)
            )
            .toMap,
          env
        )
